...
async function prepareOperation(companion, toolName, args, editSessionId = null) {
  ...
  const type = ({
    copy_all_texts: "project.copyAllTexts", create_project: "project.create", duplicate_project: "project.duplicate", open_project: "project.open", update_project: "project.update", move_project: "project.move", delete_project: "project.delete",
...
export async function createCarouselBotMcpServer(companion) {
  ...
  register("copy_all_texts", "Return all project text ready to copy: one row per text layer in slide/text-layer order, with one blank row between slides. Internal line breaks become spaces; empty text and slides are skipped. Returns text without writing the system clipboard.", z.object(targetProject).strict(), (args) => browserOperation(companion, "copy_all_texts", args), { readOnlyHint: true, idempotentHint: true });
